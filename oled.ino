...
void loadDisplay() {
  ...
  for(i = icon_element_counter - 1; i >= 0 ; i--){
    if (icon_element[i].ref != invisible){
      display.drawBitmap(icon_element[i].x, icon_element[i].y, icons[icon_element[i].ref], 16, 16, SSD1306_WHITE);
    }
  }

  for (i = 0; i < element_counter; i++) {
    if (element[i].font != element[i - 1].font || i == 0) {
      display.setTextSize(element[i].font);
    }
    display.setCursor(element[i].x, element[i].y); 
    display.print(element[i].data);
  }
  for (i = 0; i < special_element_counter; i++) {
    switch (special_element[i].type) {
      case rect: {
          display.drawLine(special_element[i].x, special_element[i].y, special_element[i].width, special_element[i].height, SSD1306_WHITE);
          break;
        }
      case circle: {
          display.drawCircle(special_element[i].x, special_element[i].y, special_element[i].width, SSD1306_WHITE);
          break;
        }
      case filledCircle: {
          display.fillCircle(special_element[i].x, special_element[i].y, special_element[i].width, SSD1306_WHITE);
          break;
        }
      case rectangle: {
          display.drawRect(special_element[i].x, special_element[i].y, special_element[i].width, special_element[i].height, SSD1306_WHITE);
          break;
        }
      case filledRectangle: {
          display.fillRect(special_element[i].x, special_element[i].y, special_element[i].width, special_element[i].height, SSD1306_WHITE);
          break;
        }
      case roundRectangle: {
          display.drawRoundRect(special_element[i].x, special_element[i].y, special_element[i].width, special_element[i].height, 4, SSD1306_WHITE);
          break;
      }
      case filledRoundRectangle: {
          display.fillRoundRect(special_element[i].x, special_element[i].y, special_element[i].width, special_element[i].height, 4, SSD1306_WHITE);
          break;
      }
    }
  }
...
void createList(byte offset_x, byte offset_y, bool selector) {
  ...
  for (d = 0; d < n_rows; d++) {
    newDisplayElement(offset_x, d * 64 / n_rows + offset_y, elements_list[d]);
  }
  if (details_list){
    for (d = 0; d < n_rows; d++){
      if (details_elements_list[d].type == txt){
        newDisplayElement(124, d * 64 / n_rows + offset_y, String(details_elements_list[d].data));
      }
      else {
...
void setParameter(int address, int min, int max){
  ...
  parameter.max = max;
  if (max == 1){
    newDisplayElement(124, element[element_selected % n_rows + begin_list].y, parameter.data ? "ON" : "OFF");
  }
  else {
    newDisplayElement(124, element[element_selected % n_rows + begin_list].y, String(parameter.data));
  }
}
